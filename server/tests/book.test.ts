...
describe(`Book APIs ("${baseUrl}")`, () => {
	...
	describe.skip('Refuse book transfer ("DELETE /transfer/:id")', () => {
		test("Ok", async () => {
			const user = await insertUser();
			const otherUser = await insertUser("OtherUser");
			...
			const cookie = await login();
			const res = request(app).delete(`${baseUrl}/transfer/${book}`).set("Cookie", cookie);
		...
		test("Invalid book id", async () => {
			...
			const res = request(app)
				.delete(`${baseUrl}/transfer/${book + 1}`)
				.set("Cookie", cookie);
		...
		test("User not logged in", async () => {
			...
			const res = request(app).delete(`${baseUrl}/transfer/${book}`);
